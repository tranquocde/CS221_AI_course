...
def create_chain_csp(n):
    ...
    for index in range(1,n):
        csp.add_binary_factor(variables[index],variables[index-1],factor_func=lambda x,y: x!=y)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for index,var in enumerate(variables):
        csp.add_variable(var,domain=[(index,i) for i in range(n)])
    for var_1 in variables:
        for var_2 in variables:
            if var_1!=var_2:
                csp.add_binary_factor(var_1,var_2,factor_func=lambda x,y:x[1]!=y[1])
                csp.add_binary_factor(var_1,var_2,factor_func=lambda x,y:x[1]-y[1]!=x[0]-y[0])
                csp.add_binary_factor(var_1,var_2,factor_func=lambda x,y: x[1]-y[1]!=-x[0]+y[0])
...
class BacktrackingSearch():
    ...
    def arc_consistency_check(self, var):
        ...
        # raise Exception("Not implemented yet")
        def rm_inconsistency(i,j):
            rm_item_of_i = []
            for val1 in self.domains[i]:
                #set value for variable i equals val1
                flag = False #this var is to check whether we can find available value for var j
                for val2 in self.domains[j]:
                    if self.csp.binaryFactors[i][j][val1][val2] > 0: flag = True
                    #so then found the value for variable j
                if flag == False:
                    rm_item_of_i.append(val1)
            if len(rm_item_of_i)>0:
                for item in rm_item_of_i:
                    self.domains[i].remove(item)
                return True
            else:
                return False

        frontier = [(j,var) for j in self.csp.get_neighbor_vars(var)]
        while frontier:
            var_k,var_j = frontier.pop()
            if rm_inconsistency(var_k,var_j):
                for var_l in self.csp.get_neighbor_vars(var_k):
                    if var_l != var_k:
                        frontier.append((var_l,var_k))
