...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        if deckCardCounts is None:
            ...
            if nextCardIndexIfPeeked is not None: # the case we know the next card
...
def blackjackFeatureExtractor(state: Tuple, action: str) -> List[tuple]:
    ...
    # -> list of (feature key, feature value)
    features=[]
    #indicator for the action and the current totals
    featureKey=(total,action)
    featureValue=1
    features.append((featureKey,featureValue))
    if counts is not None:
        countsList=list(counts)
        # indicator for the action and number of cards remaining with each face value
        for index,item in enumerate(counts):
            featureKey=(index,item,action)
            featureValue=1
            features.append((featureKey, featureValue))
            if item>0:
                countsList[index]=1
        #indicator for the action and the presence/absence of each face value in the deck
        featureKey=(tuple(countsList),action)
        featureValue=1
        features.append((featureKey,featureValue))
    return features
    # END_YOUR_CODE
